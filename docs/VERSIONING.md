...
本文档定义 Aurora 语言的版本管理与兼容性承诺。自 v1.0.0 起生效。
...
| 级别 | 含义 | 变更许可 |
| --- | --- | --- |
| stable | v1.0.0 规范承诺的行为 | 仅 MAJOR 版本可破坏 |
...
1. 所有变更必须通过测试套件:`python3 -m unittest discover -s aurora/tests`(当前 149 项);
2. 破坏性变更须同时更新 `docs/SPEC.md`、`CHANGELOG.md` 与示例;
3. 版本号同步更新于:`aurora/__init__.py`、`ide/static/index.html`(状态栏)、`vscode-aurora/package.json`;
