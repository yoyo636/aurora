...
| v3.3.0 | 2026-09-18 | 语言级原生增量计算引擎:source/live/transact 三关键字、Source<T>/Live<T> 两类型;编译期纯度检查 + 循环依赖检测;拉模式脏标记与惰性重算;哈希短路避免无效级联;subscribe 订阅回调;transact 批量事务;依赖图导出(Mermaid/DOT)与重算统计;新增 incremental.py 与 30+ 测试;21 项评审修复;稳定性:experimental(增量计算为新特性) |
