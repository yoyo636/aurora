...
### 评审修复(21 项)
1. **source 写入语义统一**:`source(value)` 初始化与 `node = value` 赋值走同一 version 计数器,避免首次写入不触发下游脏标记
2. **live 块纯度检查增强**:类型检查器现在能捕获块内对外层 `var` 的赋值,编译期报错而非运行时静默通过
3. **循环依赖检测完善**:Computing 状态改为节点级而非全局,允许同一节点在不同分支并行重算
4. **哈希短路边界修复**:结果为 nil / 0 / "" 时不再误判为"无变化",结构化哈希正确覆盖 falsy 值
5. **transact 嵌套支持**:嵌套 `transact` 块现在正确合并为最外层事务,不再中间态提前传播
6. **subscribe 回调时序**:回调在哈希短路后才触发,避免结果未变时空通知
7. **大对象引用追踪阈值**:1KB 阈值改为可配置,默认值从 512B 调到 1KB,减少序列化开销
8. **依赖图导出 Mermaid 格式修复**:节点 ID 中的特殊字符转义,箭头方向与实际依赖关系一致
9. **get_stats() 计数精度**:修复重算计数器在 transact 批量更新时多算的问题
10. **Source<T> 类型标注**:类型检查器现在接受 `Source<int>` / `Live<float>` 标注,编译期校验赋值类型
11. **live 块内 source 读取**:修复在 live 块内读取 source 时依赖追踪遗漏的边界情况
12. **未读节点不重算验证**:确认 transact 块结束后只有被读取过的 live 节点才进入重算队列
13. **增量计算与并发交互**:`spawn` 协程中读取 live 节点时正确触发惰性重算,不出现竞态
14. **错误消息格式统一**:所有 IncrementalError 遵循 `L行:C列: [类别] 描述` 格式
15. **export_graph DOT 兼容性**:DOT 输出通过 `dot -Tsvg` 渲染验证,无语法错误
16. **内存泄漏修复**:订阅回调解绑后正确清理监听器列表,防止闭包引用泄漏
17. **source 删除语义**:修复 source 节点被垃圾回收后下游 live 节点悬挂引用的问题
18. **测试覆盖补充**:为哈希短路 falsy 值、transact 嵌套、并发读取三个边界场景补充专项测试
19. **文档一致性**:README 增量计算章节示例与实际行为对齐,`count = 5` 注释改为"只标记脏"
20. **CLI 调试命令**:`aurora debug --incremental` 支持 `--graph mermaid/dot` 与 `--stats` 子选项
21. **性能回归基准**:确保增量计算引入后,非增量路径(source/live 未使用)的程序启动时间与 v3.2.0 持平
